...
void test_main (void) {
    ...
    const uint32_t wr_data0[] = {0x11111111, 0x22222222};
    const uint32_t wr_data1[] = {0x44444444, 0x88888888};
    const uint32_t exp_data[] = {
    ...
    for (uint32_t addr = part.address;
            addr < part.digest_address;
            addr += part.granularity / 8) {
        dai_wr(addr, wr_data1[0], wr_data1[1], part.granularity, 0);
    }

    // Step 2: Check that reading the data fuses now returns an
    // integrity error.
    if (part_read_compare(&part, exp_data,
                OTP_CTRL_STATUS_CPTRA_SS_LOCK_HEK_PROD_0_ERROR_MASK, 0) != 0) {
